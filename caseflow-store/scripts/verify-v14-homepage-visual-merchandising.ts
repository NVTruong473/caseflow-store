...
const TASK_ID = "V14-T05";
const ARTIFACT_DIR = path.join(".agent", "artifacts", "v14-t05");
...
const VIEWPORTS = [
  {
    ...
    name: "mobile-vi",
    screenshotName: "home-v14-mobile-vi.png",
    viewport: { height: 900, width: 375 },
  },
  {
    language: "en" as const,
    name: "tablet-en",
    screenshotName: "home-v14-tablet-en.png",
    viewport: { height: 1000, width: 768 },
  },
  {
    language: "en" as const,
    name: "desktop-en",
    screenshotName: "home-v14-desktop-en.png",
...
async function main() {
  ...
  try {
    ...
    const pass = {
      ...
      layoutVariety: viewportChecks.every((check) => check.pass.layoutVariety),
...
async function inspectViewport(
  ...
  const layoutKinds = await page
    .locator("[data-v14-merchandising-layout]")
    .evaluateAll((elements) =>
      Array.from(
        new Set(
          elements
            .map((element) => element.getAttribute("data-v14-merchandising-layout"))
            .filter(Boolean),
        ),
      ).sort(),
    );
  ...
  return {
    ...
    pass: {
      ...
      layoutVariety:
        layoutKinds.includes("category-spine-rail") &&
        layoutKinds.includes("deal-strip") &&
        layoutKinds.includes("editorial-feature") &&
        layoutKinds.includes("reading-path") &&
        layoutKinds.includes("translation-pairs") &&
        layoutKinds.length >= 5,
      localImagesOnly: localImageSourceProblems.length === 0,
      noOverflow: !layout.hasHorizontalOverflow,
      productLinksPreserved: counts.productLinks >= 18,
...
async function waitForHomepage(page: Page) {
  ...
  await page.locator("[data-v14-merchandising-layout='editorial-feature']").waitFor();
}
...
async function readCounts(page: Page) {
  return {
    ...
    v14Layouts: await page.locator("[data-v14-merchandising-layout]").count(),
  };
}
