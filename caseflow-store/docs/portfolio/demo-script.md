...
|---|---|---|
| `00:00-00:17` | Product context | CaseFlow Books v1.18.3 title card |
| `00:17-01:51` | Customer journey | Production storefront, discovery, Buy Now, QR practice, official order, order history |
| `01:51-02:30` | Operations | Production dashboard, order lookup, status transition |
| `02:30-03:15` | Architecture | Six-layer modular-monolith map |
| `03:15-03:34` | Evidence and boundaries | Repository, QA, and buyer handoff |

The canonical narration text and pacing limits are stored in
[`demo-scenes.json`](demo-scenes.json). The rendered timing and audio duration
for every scene are stored in
[`assets/demo-v1.18.3/render-report.json`](assets/demo-v1.18.3/render-report.json).
...
scene captions, retimes long screen recordings within a bounded playback rate,
generates a sample-free background score, ducks music beneath speech,
normalizes H.264/AAC media, removes inherited metadata, and verifies final
duration and resolution. Narration is synthetic and disclosed in the portfolio
index.
